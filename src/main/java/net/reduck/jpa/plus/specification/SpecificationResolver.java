...
class SpecificationResolver<T> implements Specification<T> {
    ...
    private Predicate resolve(AttributeProjectionDescriptor condition, CriteriaBuilder criteriaBuilder, Root<T> root) {
        ...
        switch (condition.operatorType) {
            ...
            case NOT_CONTAINS:
                if (condition.ignoreCase) {
                    predicate = (criteriaBuilder.notLike(criteriaBuilder.lower(getPath(condition, root)), "%" + escapeCharacter.escape(((String) condition.value).toLowerCase()) + "%", escapeCharacter.getEscapeCharacter()));
                } else {
                    predicate = criteriaBuilder.notLike(getPath(condition, root), "%" + escapeCharacter.escape(String.valueOf(condition.value)) + "%", escapeCharacter.getEscapeCharacter());
                }
                break;
